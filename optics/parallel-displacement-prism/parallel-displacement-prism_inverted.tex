...
\begin{tikzpicture}[inverted,inverted]
  ...
  % 
  \draw[fill=cyan!10] ({-\Width/2+1},0) coordinate (BL) rectangle ++ ({\Width},{-\Height});
  \coordinate (O) at (0,0);
  ...
  \draw[<->] (BL)++(-0.2,0) -- ++(0,{-\Height}) node[midway,left] {$b$};
  % 
  \draw[<->] (B)++({-90+\AngleA}:{\Width*0.4}) coordinate (D1) -- ++({\AngleA}:{\ParallelDistance}) coordinate (D2) node at ($(D1)!0.5!(D2)!-0.5em!90:(D2)$) {$d$};
  % 
