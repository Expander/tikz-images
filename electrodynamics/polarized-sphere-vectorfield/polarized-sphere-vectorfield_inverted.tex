...
\begin{tikzpicture}[inverted,inverted]
  ...
  \pgfmathsetmacro{\R}{1} % radius of sphere
  \pgfmathsetmacro{\epsr}{2} % dielectric constant
  \pgfmathsetmacro{\Ei}{1} % field strength inside sphere
  \begin{axis}[inverted,
    view={0}{90},
    axis equal image,
    xmin={-\xmax}, xmax={\xmax},
    ...
    samples=15,
    samples y=15,
    colormap={whiteblack}{rgb=(0.9,0.9,0.9) rgb=(1,0,0)},
    declare function={
      rr(\x,\y) = sqrt(\x*\x + \y*\y);
      theta(\x,\y) = atan2(\y,\x);
      ...
      Ea(\x,\y) = sqrt(Ez(\x,\y)^2 + Ex(\x,\y)^2); % magnitude
    },
    ]
    % vector field
    \addplot3[
      -{Latex[scale=0.75]},
      quiver={
        u={ifthenelse(rr(x,y)<\R, 0, Ex(x,y)/Ea(x,y))}, % normalized vectors
        v={ifthenelse(rr(x,y)<\R, \Ei, Ez(x,y)/Ea(x,y))}, % normalized vectors
        scale arrows=0.3,
      },
      quiver/colored,
      point meta={ifthenelse(rr(x,y)<\R, \Ei, Ea(x,y))}, % coloring according to magnitude
      domain=-3:3,
      domain y=-3:{3-2/15-0.3},
    ] ({x},{y},0);
    % sphere
    \addplot[white,very thick,domain=0:360,samples=100] ({\R*cos(x)}, {\R*sin(x)});
    % capacitor
