...
\begin{tikzpicture}[inverted,inverted]
  ...
  % outlet
  \draw[thick,fill=gray!60] (0,{\ymax}) circle ({0.5*\width} and {0.1*\width});
  % boundary
  \pgfmathsetmacro{\xminboundary}{sqrt((\xmax)^2*\ymax/\ymin)};
  \draw[thick,domain=\xminboundary:\xmax] plot[samples=50] function{(\xmax/x)*(\xmax/x)*\ymax};
  \draw[thick,domain=\xmin:-\xminboundary] plot[samples=50] function{(\xmax/x)*(\xmax/x)*\ymax};
  % vector field
  \foreach \NX in {1,...,{\nxarrows}} {%
    \foreach \NY in {1,...,{\nyarrows}} {%
